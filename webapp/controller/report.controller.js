sap.ui.define([
	...
	return BaseController.extend("nus.edu.sg.ot_non_ea_detailedreport.controller.report", {
		formatter: Formatter,
		onInit: function () {
		...
		handleValueHelpRequestId: function (oEvent) {
			...
			aFilter.push(new sap.ui.model.Filter("CLAIM_TYPE", sap.ui.model.FilterOperator.EQ, '104'));
		...
		openClaimNoValueHelpPopUp: function () {
			...
			if (!this._oDialogAddRequestId) {
				this._oDialogAddRequestId = Fragment.load({
					id: oView.getId(),
					name: "nus.edu.sg.ot_non_ea_detailedreport.fragment.RequestIdValueHelpDialog",
					controller: this
						...
						if (NoSearchHelpPopUp !== 'Y') {
							...
							if (!that._oDialogAddUlu) {
								that._oDialogAddUlu = Fragment.load({
									id: oView.getId(),
									name: "nus.edu.sg.ot_non_ea_detailedreport.fragment.UluValueHelpDialog",
									controller: that
								}).then(function (oDialog) {
									oView.addDependent(oDialog);
									return oDialog;
			...
			if (NoSearchHelpPopUp !== 'Y') {
				...
				if (!this._oDialogAddFdlu) {
					this._oDialogAddFdlu = Fragment.load({
						id: oView.getId(),
						name: "nus.edu.sg.ot_non_ea_detailedreport.fragment.FdluValueHelpDialog",
						controller: this
					}).then(function (oDialog) {
						oView.addDependent(oDialog);
						return oDialog;
				...
				oDataModel.read("/StaffListViews", {
					...
					success: function (oData) {
						if (oData) {
							...
							// 		id: oView.getId(),
							// 		name: "nus.edu.sg.ot_non_ea_detailedreport.fragment.StaffValueHelpDialog",
							// 		controller: that
							// 	}).then(function (oDialog) {
							// 		oView.addDependent(oDialog);
							// 		return oDialog;
						...
						if (oData.results.length) {
							...
							if (!this._oDialogVerifer) {
								this._oDialogVerifer = Fragment.load({
									id: oView.getId(),
									name: "nus.edu.sg.ot_non_ea_detailedreport.fragment.VerifierValueHelpDialog",
									controller: this
								}).then(function (oDialog) {
									oView.addDependent(oDialog);
									return oDialog;
						...
						if (oData.results.length) {
							...
							if (!this._oDialogApprover) {
								this._oDialogApprover = Fragment.load({
									id: oView.getId(),
									name: "nus.edu.sg.ot_non_ea_detailedreport.fragment.ApproverValueHelpDialog",
									controller: this
								}).then(function (oDialog) {
									oView.addDependent(oDialog);
									return oDialog;
		...
		searchFilter: function (oEvent) {
			...
			if (this.AppModel.getProperty("/claimRequest/UluList").length > 0 || userRoleGrp === "NUS_CHRS_ECLAIMS_SUPER_ADMIN") {
				...
				if (!!selectedItemsUlu && selectedItemsUlu.length > 0) {
					...
					andFilter.push(new sap.ui.model.Filter("PROCESS_CODE", FilterOperator.EQ, "104"));
		...
		_fnHandleDataToExport: function (aData) {
			...
			oSettings = {
				...
				dataSource: aData,
				fileName: "Overtime(NON EA) Detailed Report.xlsx"
			};

			oSheet = new Spreadsheet(oSettings);
			oSheet.build()
				.then(function () {
					MessageToast.show('Overtime(NON EA) Detailed Report exported successfully..!!');
				})
				.finally(function () {
					oSheet.destroy();
				});
